 ...
 *
 * Wraps:
 *   • Agora Web RTC SDK v4.x  (AgoraRTC global, lazy-loaded on first start())
...
class CityHealthAgora {
   ...
   * The Agora Web SDK is lazy-loaded on the first call so it never
   * triggers camera/mic permission prompts on the home screen.
   * @param {object} [opts]
  ...
  async start(opts = {}) {
    ...
    // ── 0. Lazy-load the Agora Web RTC SDK ─────────────────────
    //       Deferred until start() so the SDK's internal device
    //       enumeration (which triggers the browser camera/mic
    //       permission prompt) only happens on the conversation
    //       screen, not on the home page.
    try {
      await CityHealthAgora._loadSDK();
    } catch (sdkErr) {
      this._emit('error', { message: 'Failed to load Agora RTC SDK. Check your network connection.' });
      this._setState('idle');
      return;
    }

    // Enable Agora Web SDK log upload for the user/client side.
    CityHealthAgora.enableLogUpload();

    try {
      // ── 1. Wait for App ID to finish loading from /api/config ─
      ...
      // ── 2. Request microphone permission FIRST (before anything else)
      ...
      // ── 7. Create + publish local microphone track ──────────
      ...
      // ── 8. Start Agora Conversational AI agent via backend ──
  ...
  /* ============================================================
     Lazy-load the Agora Web RTC SDK
     Call once; subsequent calls return the cached promise.
     ============================================================ */
  static _loadSDK() {
    // Already loaded
    if (typeof AgoraRTC !== 'undefined') {
      CityHealthAgora._sdkReady = Promise.resolve();
      return CityHealthAgora._sdkReady;
    }
    // Already loading — return the in-flight promise
    if (CityHealthAgora._sdkReady) return CityHealthAgora._sdkReady;

    const AGORA_SDK_URL = 'https://download.agora.io/sdk/release/AgoraRTC_N-4.24.3.js';
    console.log('[CityHealthAgora] Lazy-loading Agora RTC SDK…');

    CityHealthAgora._sdkReady = new Promise((resolve, reject) => {
      const script = document.createElement('script');
      script.src = AGORA_SDK_URL;
      script.onload = () => {
        console.log('[CityHealthAgora] Agora RTC SDK loaded ✓');
        resolve();
      };
      script.onerror = () => reject(new Error('Failed to load Agora RTC SDK from ' + AGORA_SDK_URL));
  ...
  _startVolumeMonitor() {
    ...
    this._volInterval = setInterval(() => {
      ...
      const vol = this._remoteTrack.getVolumeLevel();
