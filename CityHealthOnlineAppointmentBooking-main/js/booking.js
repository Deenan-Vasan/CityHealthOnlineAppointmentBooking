...
function runDbAnimation() {
  ...
  tick(4000, () => {
    ...
    // In live mode the AI agent already spoke the real confirmation — do NOT
    // overwrite the transcript with hardcoded demo text.  Only set the fallback
    // bubble in demo mode (or as a last-resort fallback using real booking data).
    if (!isLiveMode()) {
      ...
      // Build the fallback bubble from actual confirmed booking data so it is
      // accurate if the live transcript hasn't populated the bubble yet.
      const liveDoc  = confirmedDoctor.name || 'your doctor';
      const liveDate = confirmedDateTime.date || 'your appointment date';
      const liveTime = confirmedDateTime.time || 'your appointment time';
      const liveFee  = confirmedDoctor.fee ? (' Fee is ' + confirmedDoctor.fee + ', payable at the clinic.') : '';
      setAgentText(5, 'All done! Your appointment with ' + liveDoc + ' is confirmed for ' + liveDate + ' at ' + liveTime + '.' + liveFee + ' You\'ll receive reminders 24 hours, 2 hours, and 30 minutes before your appointment. Is there anything else I can help you with?');
    }
...
async function openBookingFlow() {
  ...
  // Mute the widget agent's mic so it cannot hear the user during the
  // booking flow and will not generate spurious English responses.
  try {
    if (window.cityHealthAgent && typeof window.cityHealthAgent.setMicMuted === 'function') {
      window.cityHealthAgent.setMicMuted(true);
      console.log('[BookingFlow] Widget agent mic muted for booking session.');
    }
  } catch (e) {
    console.warn('[BookingFlow] Could not mute widget agent mic:', e.message);
  }
...
async function closeBookingFlow() {
  ...
  // Restore widget agent mic now that the booking session is over.
  try {
    if (window.cityHealthAgent && typeof window.cityHealthAgent.setMicMuted === 'function') {
      window.cityHealthAgent.setMicMuted(false);
      console.log('[BookingFlow] Widget agent mic restored after booking session.');
    }
  } catch (e) {
    console.warn('[BookingFlow] Could not unmute widget agent mic:', e.message);
  }
}
